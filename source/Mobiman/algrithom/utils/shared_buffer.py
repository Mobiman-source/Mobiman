...
from tgnmappo.utils.util import get_shape_from_obs_space, get_shape_from_act_space
...
class SharedReplayBuffer(object):
    ...
    def __init__(self, args, num_agents, obs_space, cent_obs_space, act_space):
        ...
        if act_space.__class__.__name__ == 'Discrete':
            ...
            sum_dims = int(np.sum(action_dims))
            self.available_actions = np.ones((self.episode_length + 1, self.n_rollout_threads, num_agents, sum_dims),
                                             dtype=np.float32)
        else:
